...
class TramiteController extends Controller
{
    ...
    public function verHistorico($id)
    {
        try {
            ...
            // Verificar que el usuario tenga permisos para ver este trámite
            $user = auth()->user();
            
            // Los administradores y revisores pueden ver todos los trámites
            if (!$user->hasRole(['admin', 'revisor'])) {
                // Los proveedores solo pueden ver sus propios trámites
                if ($user->hasRole('proveedor') && $tramite->proveedor->rfc !== $user->rfc) {
                    abort(403, 'No tienes permisos para ver este trámite');
                }
            }

            return view('tramites.historico', compact('tramite'));

        } catch (\Exception $e) {
            Log::error('Error al mostrar trámite histórico', [
                'tramite_id' => $id,
                'user_id' => auth()->id(),
                'error' => $e->getMessage()
            ]);
            ...
            return redirect()->back()
                ->with('error', 'No se pudo cargar el trámite solicitado');
        }
    }
} 