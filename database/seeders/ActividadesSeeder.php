...
class ActividadesSeeder extends Seeder
{
    ...
    private function poblarDesdeDenueJson($jsonPath): void
    {
        try {
            ...
            foreach ($data['sectores'] as $sector) {
                DB::table('catalogo_sectores')->insertOrIgnore([
                    'nombre' => $sector['nombre'],
                ]);
            }
            
            $sectoresDB = DB::table('catalogo_sectores')->get();
            $actividadesLote = [];
            $sectorIndex = 0;
            ...
            foreach ($data['actividades'] as $actividad) {
                ...
                if (count($actividadesLote) >= 1000) {
                    DB::table('catalogo_actividades')->insert($actividadesLote);
                    $actividadesLote = [];
                }
            }
    ...
    private function poblarDesdeJsonLocal(): void
    {
        try {
            ...
            $actividades = [];
            $sectores = DB::table('catalogo_sectores')->get();
            ...
            if (!empty($actividades)) {
                ...
                foreach ($chunks as $chunk) {
                    DB::table('catalogo_actividades')->insert($chunk);
                }
    ...
    private function crearSectoresBasicos(): void
    {
        ...
        $sectorCount = DB::table('catalogo_sectores')->count();
        
        if ($sectorCount == 0) {
            DB::table('catalogo_sectores')->insert($sectores);
