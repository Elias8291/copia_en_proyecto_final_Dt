@props(['editable' => false, 'archivosRequeridos' => [], 'tipoPersona' => 'Física', 'archivosCargados' => null, 'estadosArchivos' => [], 'comentariosArchivos' => [], 'soloLectura' => false])
...
<div class="space-y-6" {{ $attributes }}>
    <div class="flex items-center space-x-3 mb-6">
        ...
        <div>
            <h3 class="text-lg font-semibold text-gray-900">Corrección de Archivos</h3>
            <p class="text-sm text-gray-500">Documentos que requieren corrección para {{ $tipoPersona === 'Física' ? 'Persona Física' : 'Persona Moral' }}</p>
        </div>
    </div>
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-[#9D2449]/80">
                        <p>Solo los archivos marcados como "Rechazado" permiten subir un nuevo documento para corrección.</p>
            </div>
            </div>
            </div>
    </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($archivosFiltrados as $archivo)
                @php
                    ...
                    $archivoNombre = is_array($archivo) ? ($archivo['nombre'] ?? '') : ($archivo->nombre ?? '');
                    $archivoDescripcion = is_array($archivo) ? ($archivo['descripcion'] ?? '') : ($archivo->descripcion ?? '');
                    $archivoTipo = is_array($archivo) ? ($archivo['tipo_archivo'] ?? 'pdf') : ($archivo->tipo_archivo ?? 'pdf');
                    
                    // Obtener estado del archivo
                    $estadoArchivo = $estadosArchivos[$archivoId] ?? 'Pendiente';
                    $esEditableArchivo = $estadoArchivo === 'Rechazado';
                    
                    // Obtener comentario del archivo
                    $comentarioArchivo = $comentariosArchivos[$archivoId] ?? '';
                    
                    // Obtener archivo cargado si existe
                    $archivoCargado = null;
                    if ($archivosCargados && is_array($archivosCargados)) {
                        $archivoCargado = collect($archivosCargados)->first(function($ac) use ($archivoId) {
                            return $ac['catalogo_archivo_id'] == $archivoId;
                        });
                    }
                    
                    $tiposMime = ArchivosHelper::obtenerTiposMimePorTipoArchivo($archivoTipo);
                    $tiposPermitidos = ArchivosHelper::obtenerTiposPermitidosLegibles($archivoTipo);
                ...
                <div class="{{ $bgColor }} border-2 border-dashed {{ $borderColor }} rounded-lg p-4 hover:border-[#9D2449] transition-colors @if(!$esEditableArchivo && $editable) opacity-50 @endif">
                    <div class="text-center">
                        ...
                        <h5 class="font-medium text-gray-900 mb-2 text-sm">
                                    {{ $archivoNombre }}
                                    @if($esEditableArchivo)
                                <span class="text-red-500">*</span>
                                    @endif
                                </h5>
                        
                        <!-- Descripción -->
                        <p class="text-xs text-gray-500 mb-3 line-clamp-2">{{ $archivoDescripcion }}</p>
                    ...
                    @if($archivoCargado)
                        <div class="mb-3 p-2 bg-gray-50 rounded border border-gray-200">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-xs text-gray-600 font-medium">Archivo actual:</span>
                                <span class="text-xs text-gray-700">{{ $archivoCargado['nombre_original'] ?? 'Sin nombre' }}</span>
                            </div>
                            @if($archivoCargado['ruta'])
                                <a href="{{ asset('storage/' . $archivoCargado['ruta']) }}" 
                                   target="_blank"
                        ...
                        @if($comentarioArchivo)
                            <div class="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
                                <div class="flex items-start space-x-2">
                                    ...
                                    <div class="flex-1">
                                        <h6 class="text-xs font-medium text-yellow-800 mb-1">Observaciones del Revisor:</h6>
                                        <p class="text-xs text-yellow-700">{{ $comentarioArchivo }}</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                    ...
                    @if($esEditableArchivo && $editable)
                            <label class="cursor-pointer group">
                                <input type="file"
                                    name="documentos_correccion[{{ Str::slug($archivoNombre) }}]"
                                    accept=".{{ $tiposMime }}"
                                    class="hidden {{ $errors->has('documentos_correccion.' . Str::slug($archivoNombre)) ? 'border-red-500' : '' }}"
                                    onchange="updateFileNameCorreccion(this, '{{ Str::slug($archivoNombre) }}-name-correccion')"
                    ...
                    @else
                        ...
                        <input type="hidden" 
                               name="documentos_correccion_nombres[{{ Str::slug($archivoNombre) }}]" 
                               value="{{ old('documentos_correccion_nombres.' . Str::slug($archivoNombre), '') }}">
                        
                        <!-- Nombre del archivo seleccionado -->
                        <p id="{{ Str::slug($archivoNombre) }}-name-correccion" class="text-xs text-[#9D2449] mt-2 hidden font-medium"></p>
                        
                        <!-- Error individual para cada archivo -->
                        @error('documentos_correccion.' . Str::slug($archivoNombre))
        ...
        <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div class="flex items-center">
                ...
                <p class="text-sm text-blue-700">
                    <strong>Importante:</strong> Solo los archivos marcados como "Rechazado" permiten subir un nuevo documento para corrección.
                </p>
            </div>
        </div>
            ...
            <div class="flex items-start space-x-3">
                ...
                <div>
                    ...
                    <ul class="text-xs text-yellow-700 mt-2 space-y-1">
                        <li>• Solo los archivos marcados como "Rechazado" permiten subir un nuevo documento</li>
                        <li>• Los archivos aprobados y pendientes no se pueden modificar</li>
                        <li>• Asegúrese de que los nuevos archivos cumplan con los requisitos especificados</li>
                        <li>• El archivo anterior será reemplazado por el nuevo documento</li>
                    </ul>
                </div>
            </div>
        </div>
                        ...
                        <div class="border-t border-gray-200 pt-3 mt-3">
                            <div class="mb-3">
                                ...
                                <textarea 
                                    ...
                                    rows="2"
                                    onchange="sincronizarComentarioArchivo('{{ is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '') }}')">{{ old('archivos.' . (is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '')) . '.comentario', $comentariosArchivos[is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '')] ?? '') }}</textarea>
                            </div>
                            ...
                            <input type="hidden" name="archivos[{{ is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '') }}][decision]" 
                                   ...
                                   value="{{ old('archivos.' . (is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '')) . '.decision', $estadosArchivos[is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '')] ?? 'Pendiente') }}">
                            <input type="hidden" name="archivos[{{ is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '') }}][comentario]" 
                                   ...
                                   value="{{ old('archivos.' . (is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '')) . '.comentario', $comentariosArchivos[is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '')] ?? '') }}">
                            
                            <!-- Estado del archivo -->
                            <div class="mt-2 text-center">
                                @php
                                    $archivoId = is_array($archivo) ? ($archivo['id'] ?? '') : ($archivo->id ?? '');
                                    $decisionOld = old('archivos.' . $archivoId . '.decision', $estadosArchivos[$archivoId] ?? 'Pendiente');
...
<script>
function updateFileNameCorreccion(input, nameElementId) {
    ...
    if (file) {
        ...
        nameElement.textContent = `Archivo seleccionado: ${file.name}`;
...
function restaurarNombresArchivosCorreccion() {
    const hiddenInputs = document.querySelectorAll('input[name^="documentos_correccion_nombres["]');
    hiddenInputs.forEach(hiddenInput => {
        const nombre = hiddenInput.value;
        if (nombre) {
            const archivoSlug = hiddenInput.name.match(/\[([^\]]+)\]/)[1];
            const fileNameElement = document.getElementById(`${archivoSlug}-name-correccion`);
            if (fileNameElement) {
...
function restaurarEstadosArchivosCorreccion() {
    const hiddenInputs = document.querySelectorAll('input[name$="[decision]"][name*="archivos_correccion"]');
    hiddenInputs.forEach(hiddenInput => {
        const decision = hiddenInput.value;
        const archivoId = hiddenInput.name.match(/\[([^\]]+)\]/)[1];
        const estadoElement = document.getElementById(`estado_archivo_correccion_${archivoId}`);
...
function evaluarArchivoCorreccion(archivoId, decision) {
    const textareaEl = document.getElementById(`textarea_archivo_correccion_${archivoId}`);
    const decisionEl = document.getElementById(`decision_archivo_correccion_${archivoId}`);
    const comentarioEl = document.getElementById(`comentario_archivo_correccion_${archivoId}`);
    const estadoEl = document.getElementById(`estado_archivo_correccion_${archivoId}`);
    ...
    mostrarNotificacionArchivoCorreccion(archivoId, decision, comentario);
}

function sincronizarComentarioArchivoCorreccion(archivoId) {
    const textareaEl = document.getElementById(`textarea_archivo_correccion_${archivoId}`);
    const comentarioEl = document.getElementById(`comentario_archivo_correccion_${archivoId}`);
...
function mostrarNotificacionArchivoCorreccion(archivoId, decision, comentario) {
    const mensaje = `Archivo de corrección ${archivoId} ${decision.toLowerCase()}. ${comentario ? `Comentario: ${comentario}` : ''}`;
...
document.addEventListener('DOMContentLoaded', function() {
    restaurarNombresArchivosCorreccion();
    restaurarEstadosArchivosCorreccion();
    restaurarNombresArchivos();
    restaurarEstadosArchivos();
    cargarComentariosExistentes();
});

// Función para cargar comentarios existentes desde el servidor
function cargarComentariosExistentes() {
    // Los comentarios ya se cargan desde PHP, pero podemos sincronizarlos con el sistema de revisión
    const textareas = document.querySelectorAll('[id^="textarea_archivo_"]');
    textareas.forEach(textarea => {
        const archivoId = textarea.id.replace('textarea_archivo_', '');
        if (textarea.value.trim()) {
            // Si hay un comentario existente, sincronizarlo
            sincronizarComentarioArchivo(archivoId);
        }
    });
    
    // También para archivos de corrección
    const textareasCorreccion = document.querySelectorAll('[id^="textarea_archivo_correccion_"]');
    textareasCorreccion.forEach(textarea => {
        const archivoId = textarea.id.replace('textarea_archivo_correccion_', '');
        if (textarea.value.trim()) {
            // Si hay un comentario existente, sincronizarlo
            sincronizarComentarioArchivoCorreccion(archivoId);
        }
    });
    
    // Mostrar notificación de comentarios cargados
    console.log('Comentarios existentes cargados y sincronizados');
}
</script> 