...
@php
    ...
    if ($customColors) {
        ...
        $colorClasses = match($estado) {
            'pendiente', 'en_revision', 'en_proceso' => 'bg-yellow-100 text-yellow-800',
            ...
            'por_cotejar' => 'bg-blue-100 text-blue-800',
...
<span class="inline-flex items-center {{ $sizeClasses }} rounded-full font-medium {{ $colorClasses }}">
    ...
    @elseif($estado === 'pendiente' || $estado === 'en_revision' || $estado === 'en_proceso')
        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
