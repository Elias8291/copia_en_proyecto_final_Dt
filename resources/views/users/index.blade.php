                                ...
                                <div class="flex items-center space-x-2 sm:space-x-3 md:space-x-4">
                                    ...
                                    <form id="form-delete-user-{{ $user->id }}" action="{{ route('users.destroy', $user->id) }}" method="POST" class="inline">
                                        ...
                                        <button type="button"
                                            ...
                                            data-message="¿Está seguro que desea eliminar al usuario {{ $user->name ?? 'N/A' }}? Esta acción se puede revertir posteriormente."
                    ...
                    <div class="flex space-x-2 sm:space-x-3 md:space-x-4 pt-3 sm:pt-4 md:pt-5 mt-3 sm:mt-4 md:mt-5 border-t border-gray-100">
                        ...
                        <form id="form-delete-user-mobile-{{ $user->id }}" action="{{ route('users.destroy', $user->id) }}" method="POST" class="flex-1">
                            ...
                            <button type="button"
                                ...
                                data-title="Eliminar Usuario"
                                data-message="¿Está seguro que desea eliminar al usuario {{ $user->name ?? 'N/A' }}? Esta acción se puede revertir posteriormente."
                                data-form-id="form-delete-user-mobile-{{ $user->id }}"
        ...
        try {
            ...
            document.querySelectorAll('[data-action="delete"]').forEach(function(button) {
                button.addEventListener('click', function() {
                    ...
                    showDeleteModal(title, message, formId);
                });
            });
        } catch (error) {
