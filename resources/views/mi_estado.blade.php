...
@section('title', 'Mi Estado de Proveedor - ' . ($proveedor->razon_social ?? 'N/A'))
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold text-gray-800">
                                {{ $proveedor ? $proveedor->razon_social : 'Mi Estado de Proveedor' }}
                            </h1>
                            @if($proveedor)
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <p class="text-lg font-semibold text-gray-900">{{ $historialTramites ? $historialTramites->count() : 0 }}</p>
                        <p class="text-xs text-gray-400 mt-1">RFC: {{ $rfc ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="bg-white rounded-lg shadow-md border border-gray-200 mt-6">
                <div class="flex items-center justify-between p-4 border-b border-gray-200">
                    <button type="button" onclick="toggleSection('historial')"
                        ...
                        <span class="font-medium text-gray-900">Historial de Trámites - RFC: {{ $rfc ?? 'N/A' }} ({{ $historialTramites->count() }})</span>
                            ...
                            <div class="px-4 py-3 border-b border-gray-200">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="text-right text-sm text-gray-600">
                                        <div class="font-medium">{{ $tramitesDelProveedor->count() }} trámite{{ $tramitesDelProveedor->count() !== 1 ? 's' : '' }}</div>
                                        ...
                                        <div class="flex items-start justify-between">
                                            ...
                                            <div class="flex items-center space-x-3 ml-4">
                                                ...
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    {{-- Mostrar oficios del trámite --}}
                                                    @php
                                                    $oficiosTramite = $tramite->oficios ?? collect();
                                                    // También verificar archivo legacy
                                                    $rutaOficioLegacy = storage_path("app/oficios/tramite_{$tramite->id}_oficio.pdf");
                                                    $tieneOficioLegacy = file_exists($rutaOficioLegacy);
                                                    @endphp

                                                    @if($oficiosTramite->isNotEmpty())
                                                    <div class="bg-green-50 border border-green-200 rounded-lg p-4 mt-3">
                                                        <div class="space-y-2">
                                                            @foreach($oficiosTramite as $oficio)
                                                            <div class="flex items-center justify-between p-3 bg-white border border-green-200 rounded-lg">
                                                                <div class="flex items-center space-x-3">
                                                                    ...
                                                                    <div>
                                                                        <p class="text-sm font-medium text-gray-900">{{ $oficio->numero_oficio ?? 'Oficio de Aprobación' }}</p>
                                                                        <p class="text-xs text-gray-500">{{ $oficio->fecha_oficio ? $oficio->fecha_oficio->format('d/m/Y') : ($oficio->created_at ? $oficio->created_at->format('d/m/Y') : 'Sin fecha') }}</p>
                                                                    </div>
                                                                </div>
                                                                @if($oficio->url)
                                                                <a href="{{ $oficio->url }}"
                                                                    ...
                                                                    title="Descargar {{ $oficio->numero_oficio ?? 'Oficio' }}"
                                                                    target="_blank">
                                                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                ...
                                                                @elseif(isset($oficio->tramite_id))
                                                                <a href="{{ route('tramites.descargar-oficio', $oficio->tramite_id) }}"
                                                                    class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors"
                                                                    title="Descargar {{ $oficio->numero_oficio ?? 'Oficio' }}"
                                                                    target="_blank">
                                                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    @elseif($tieneOficioLegacy)
                                                    {{-- Fallback para oficios legacy --}}
                                                    <div class="bg-green-50 border border-green-200 rounded-lg p-4 mt-3">
                                                        <div class="flex items-center justify-between p-3 bg-white border border-green-200 rounded-lg">
                                                            <div class="flex items-center space-x-3">
                                                                <svg class="w-5 h-5 text-green-600" fill="currentColor" viewBox="0 0 24 24">
                                                                ...
                                                                <div>
                                                                    <p class="text-sm font-medium text-gray-900">Oficio de Aprobación</p>
                                                                    <p class="text-xs text-gray-500">Documento oficial</p>
                                                                </div>
                                                            </div>
                                                            <a href="{{ route('tramites.descargar-oficio', $tramite->id) }}"
                                                                class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors"
                                                                title="Descargar Oficio de Aprobación">
                                                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                                </svg>
                                                                PDF
                                                    ...
                                                    @endif
                                                    @else
                                                    {{-- Mostrar oficios para trámites no aprobados --}}
                                                    @php
                                                    $oficiosTramiteNoAprobado = $tramite->oficios ?? collect();
                                                    $rutaOficioLegacyNoAprobado = storage_path("app/oficios/tramite_{$tramite->id}_oficio.pdf");
                                                    $tieneOficioLegacyNoAprobado = file_exists($rutaOficioLegacyNoAprobado);
                                                    @endphp

                                                    @if($oficiosTramiteNoAprobado->isNotEmpty())
                                                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-3">
                                                        <div class="space-y-2">
                                                            @foreach($oficiosTramiteNoAprobado as $oficio)
                                                            <div class="flex items-center justify-between p-3 bg-white border border-blue-200 rounded-lg">
                                                                <div class="flex items-center space-x-3">
                                                                    ...
                                                                    <div>
                                                                        <p class="text-sm font-medium text-gray-900">{{ $oficio->numero_oficio ?? 'Oficio del Trámite' }}</p>
                                                                        <p class="text-xs text-gray-500">{{ $oficio->fecha_oficio ? $oficio->fecha_oficio->format('d/m/Y') : ($oficio->created_at ? $oficio->created_at->format('d/m/Y') : 'Sin fecha') }}</p>
                                                                    </div>
                                                                </div>
                                                                @if($oficio->url)
                                                    ...
                                                    @elseif($tieneOficioLegacyNoAprobado)
                                                    {{-- Fallback para oficios legacy no aprobados --}}
                                                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-3">
                                                        <div class="flex items-center justify-between p-3 bg-white border border-blue-200 rounded-lg">
                                                            <div class="flex items-center space-x-3">
                                                                <svg class="w-5 h-5 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                                                                ...
                                                                <div>
                                                                    <p class="text-sm font-medium text-gray-900">Oficio del Trámite</p>
                                                                    <p class="text-xs text-gray-500">Documento oficial</p>
                                                                </div>
                                                            </div>
                                                            <a href="{{ route('tramites.descargar-oficio', $tramite->id) }}"
                                                                class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors"
                                                                title="Descargar Oficio del Trámite">
                                                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                                </svg>
                                                                PDF
                                                    ...
                                                    <a href="{{ route('tramites.estado') }}"
                                                        class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-gradient-to-r from-primary to-primary-dark rounded-lg hover:shadow-lg transition-all duration-200 transform hover:-translate-y-0.5 mt-3">
                                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                        </svg>
