...
@section('content')
    ...
    <script src="/js/modules/sat-handler.js"></script>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                // Verificar que SATHandler esté disponible
                if (typeof SATHandler === 'undefined') {
                    throw new Error('SATHandler no está disponible');
                }
                
                // Inicializar SATHandler
                satHandler = new SATHandler(config);
                
                // Esperar a que se inicialice completamente
                await new Promise(resolve => {
                    const checkInit = () => {
                        if (satHandler.qrHandler) {
                            resolve();
                        } else {
                ...
                // Configurar callback personalizado para datos escaneados
                const originalHandleScannedData = satHandler.handleScannedData.bind(satHandler);
                satHandler.handleScannedData = (data) => {
                    // Llamar al método original
                    originalHandleScannedData(data);
        ...
        window.showSatModal = function() {
            ...
            if (modal && satHandler && satHandler.qrHandler) {
                modal.style.display = 'flex';
                document.body.style.overflow = 'hidden';

                const result = satHandler.qrHandler.showSatData();
                if (result.success) {
        ...
        document.getElementById('document').addEventListener('change', async (event) => {
            ...
            try {
                ...
                await satHandler.handleFile(file);
        ...
        function resetUpload() {
            ...
            if (satHandler && satHandler.qrHandler) satHandler.qrHandler.reset();
        ...
        function togglePassword(inputId) {
            ...
            if (input.type === 'password') {
                ...
                input.type = 'password';
                icon.innerHTML =
                    `<path stroke-linecap="round" stroke-linejoin="r" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>`;
            }
        }
    </script>
@endsection
