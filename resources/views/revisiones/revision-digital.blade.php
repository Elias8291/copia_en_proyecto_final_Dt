...
<div class="p-3 sm:p-4 md:p-5 lg:p-6 xl:p-8">
    <div class="max-w-7xl mx-auto bg-white shadow-sm rounded-lg border border-gray-200">        
        ...
        <div class="p-6">
            ...
            <!-- Panel de Agendamiento de Cita (se muestra después de aprobar) -->
            <div id="panel-agendar-cita" class="mt-8 hidden">
                <div class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-6">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-800">¡Trámite Aprobado!</h3>
                                <p class="text-sm text-gray-600">Ahora puede agendar una cita para revisión presencial o domiciliaria</p>
                            </div>
                        </div>
                        <button type="button" 
                                onclick="abrirModalAgendarCita({{ $tramite->id }})"
                                class="inline-flex items-center px-6 py-3 bg-[#9d2449] text-white text-sm font-medium rounded-lg hover:bg-[#8a1f40] focus:outline-none focus:ring-2 focus:ring-[#9d2449]/50 transition-all duration-200 shadow-md hover:shadow-lg">
                            ...
                            Agendar Cita de Revisión
                        </button>
                    </div>
                    
                    <!-- Información adicional -->
                    <div class="bg-white/70 backdrop-blur-sm rounded-lg p-4 mt-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            <div class="flex items-center space-x-2">
                                <div class="w-2 h-2 bg-blue-500 rounded-full"></div>
                                <span class="text-gray-700"><strong>Digital:</strong> Revisores especializados disponibles</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <div class="w-2 h-2 bg-purple-500 rounded-full"></div>
                                <span class="text-gray-700"><strong>Presencial:</strong> Cita en oficinas gubernamentales</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <div class="w-2 h-2 bg-orange-500 rounded-full"></div>
                                <span class="text-gray-700"><strong>Domiciliaria:</strong> Revisión en ubicación del negocio</span>
                            </div>
                        </div>
                        <div class="mt-3 pt-3 border-t border-gray-200/50">
                            <p class="text-xs text-gray-600">
                                <strong>📍 Ubicación base:</strong> Oaxaca de Juárez, Oaxaca - Los revisores se asignan considerando la distancia desde esta ubicación.
                            </p>
                        </div>
                    </div>
                </div>
...
<x-modals.agendar-cita-revision :tramite="$tramite" />
...
<script src="{{ asset('js/revision/agendar-cita-revision.js') }}"></script>
...
// Función para mostrar panel de agendamiento después de aprobar
function mostrarPanelAgendamiento() {
    const panel = document.getElementById('panel-agendar-cita');
    if (panel) {
...
// Función para ocultar panel de agendamiento
function ocultarPanelAgendamiento() {
    const panel = document.getElementById('panel-agendar-cita');
    if (panel) {
...
// Escuchar eventos de decisión final para mostrar/ocultar panel
document.addEventListener('DOMContentLoaded', function() {
    // Interceptar cuando se apruebe el trámite
    const originalFetch = window.fetch;
    window.fetch = function(...args) {
        return originalFetch.apply(this, args).then(response => {
            // Si es la respuesta de procesar revisión digital
            if (args[0].includes('procesar-digital')) {
                response.clone().json().then(data => {
                    if (data.success && data.decision === 'aprobado') {
                        setTimeout(() => {
                            mostrarPanelAgendamiento();
                        }, 1000);
                    } else {
                        ocultarPanelAgendamiento();
                    }
                }).catch(() => {});
            }
            return response;
