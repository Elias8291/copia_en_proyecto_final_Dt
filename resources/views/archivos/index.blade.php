    ...
    <div class="max-w-7xl mx-auto bg-white shadow-sm rounded-lg border border-gray-200">        
        <div class="p-6 border-b border-gray-200/70">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div>
                        <h1 class="text-2xl font-bold text-gray-800">Catálogo de Archivos</h1>
                        <p class="text-base text-gray-500 mt-1">Gestiona los tipos de archivos disponibles en el sistema</p>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-3 md:gap-4 lg:gap-6">
                            ...
                            <div>
                                ...
                                <select name="tipo_archivo" 
                                    ...
                                    <option value="mp3" {{ request('tipo_archivo') == 'mp3' ? 'selected' : '' }}>MP3</option>
                                    <option value="mp4" {{ request('tipo_archivo') == 'mp4' ? 'selected' : '' }}>MP4</option>
                                </select>
                            </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 md:gap-5">
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    <p class="text-xs sm:text-sm md:text-base lg:text-lg text-gray-700">
                            <span class="font-medium text-[#9d2449]">{{ $catalogoArchivos->total() }}</span> 
                            {{ $catalogoArchivos->total() == 1 ? 'catálogo encontrado' : 'catálogos encontrados' }}
                            @if($catalogoArchivos->hasPages())
                                <span class="text-gray-500 ml-1 sm:ml-2 md:ml-3">
                                    ({{ $catalogoArchivos->firstItem() }}-{{ $catalogoArchivos->lastItem() }})
                                </span>
                            @endif
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 md:gap-4">
                        ...
                        <!-- Filtros activos -->
                        @if(request()->hasAny(['search', 'tipo_persona', 'tipo_archivo', 'es_visible']))
                        <div class="flex flex-wrap items-center gap-1.5 sm:gap-2 md:gap-3">
                            ...
                            <span class="inline-flex items-center px-1.5 sm:px-2 md:px-2.5 py-0.5 sm:py-1 md:py-1.5 rounded-full text-xs sm:text-sm font-medium bg-gray-100 text-gray-700 border border-gray-200">
                                Formato: {{ strtoupper(request('tipo_archivo')) }}
                                <a href="{{ request()->fullUrlWithQuery(['tipo_archivo' => null]) }}" class="ml-1 sm:ml-1.5 text-gray-700 hover:text-gray-900">
                                    <svg class="w-2.5 h-2.5 sm:w-3 sm:h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                                    </svg>
                                </a>
                            ...
                            @if(request('es_visible') !== null && request('es_visible') !== '')
                            <span class="inline-flex items-center px-1.5 sm:px-2 md:px-2.5 py-0.5 sm:py-1 md:py-1.5 rounded-full text-xs sm:text-sm font-medium bg-gray-100 text-gray-700 border border-gray-200">
                                Estado: {{ request('es_visible') == '1' ? 'Visible' : 'Oculto' }}
                                <a href="{{ request()->fullUrlWithQuery(['es_visible' => null]) }}" class="ml-1 sm:ml-1.5 text-gray-700 hover:text-gray-900">
                                    <svg class="w-2.5 h-2.5 sm:w-3 sm:h-3" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                                    </svg>
                                </a>
        ...
        <!-- Tabla de catálogos para desktop -->
        <div class="border-t border-gray-100 overflow-hidden hidden xl:block">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            <th class="px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8 py-2 sm:py-3 md:py-4 text-left text-xs sm:text-sm font-semibold text-gray-700 uppercase tracking-wider">Catálogo</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            <td class="px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8 py-3 sm:py-4 md:py-5">
                                <div class="flex items-center space-x-2 sm:space-x-3 md:space-x-4">
                                    <div class="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 bg-[#9d2449] rounded-lg flex items-center justify-center flex-shrink-0">
                                        <svg class="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6z"/>
                                                <polyline points="14,2 14,8 20,8"/>
                                                <line x1="16" y1="13" x2="8" y2="13"/>
                                                <line x1="16" y1="17" x2="8" y2="17"/>
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="font-semibold text-gray-900 truncate max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl text-xs sm:text-sm md:text-base" title="{{ $archivo->nombre }}">
                                            {{ $archivo->nombre }}
                                    </div>
                                        <div class="text-xs sm:text-sm text-gray-500">ID: {{ $archivo->id }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8 py-3 sm:py-4 md:py-5">
                            ...
                            <td class="px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8 py-3 sm:py-4 md:py-5">
                                <span class="inline-flex items-center px-2 sm:px-2.5 md:px-3 py-1 sm:py-1.5 md:py-2 bg-green-100 text-green-800 rounded-full text-xs sm:text-sm font-medium">
                                    {{ strtoupper($archivo->tipo_archivo) }}
                                </span>
                            </td>
                            <td class="px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8 py-3 sm:py-4 md:py-5">
                                @if($archivo->es_visible)
                                    <span class="inline-flex items-center px-2 sm:px-2.5 md:px-3 py-1 sm:py-1.5 md:py-2 rounded-full text-xs sm:text-sm font-medium bg-green-100 text-green-800">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2 sm:px-2.5 md:px-3 py-1 sm:py-1.5 md:py-2 rounded-full text-xs sm:text-sm font-medium bg-red-100 text-red-800">
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293-4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                                        </svg>
                                        Oculto
                            ...
                            <td class="px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8 py-3 sm:py-4 md:py-5">
                                <div class="flex items-center space-x-2 sm:space-x-3 md:space-x-4">
                                    ...
                                    <button type="button" 
                                            onclick="showDeleteModal('Eliminar Catálogo de Archivo', '¿Estás seguro de que quieres eliminar el catálogo \"{{ $archivo->nombre }}\"? Esta acción no se puede deshacer.', 'form-delete-{{ $archivo->id }}')"
                                    ...
                                    <form id="form-delete-{{ $archivo->id }}" method="POST" action="{{ route('archivos.destroy', $archivo) }}" class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
                        ...
                        <tr>
                            <td colspan="5" class="px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8 py-8 sm:py-10 md:py-12 lg:py-16 text-center">
                                <div class="text-gray-500">
                                    ...
                                    <p class="text-xs sm:text-sm md:text-base lg:text-lg">No hay catálogos de archivos</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div class="border-t border-gray-100 pt-4 sm:pt-5 md:pt-6 lg:pt-8">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-2 sm:p-3 md:p-4 lg:p-5">
                <div class="flex items-start justify-between mb-2 sm:mb-3 md:mb-4">
                    <div class="flex items-center space-x-1.5 sm:space-x-2 md:space-x-3 lg:space-x-4 min-w-0 flex-1">
                        <div class="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 bg-[#9d2449] rounded-lg flex items-center justify-center flex-shrink-0">
                            <svg class="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 lg:h-5 lg:w-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6z"/>
                                <polyline points="14,2 14,8 20,8"/>
                                <line x1="16" y1="13" x2="8" y2="13"/>
                                <line x1="16" y1="17" x2="8" y2="17"/>
                        ...
                        <div class="min-w-0 flex-1">
                            <span class="text-gray-700 font-medium text-xs sm:text-sm md:text-base lg:text-lg block">ID: {{ $archivo->id }}</span>
                            <p class="text-xs sm:text-sm md:text-base text-gray-500 truncate">{{ $archivo->tipo_persona }}</p>
                        </div>
                    </div>
                    <div class="flex-shrink-0 ml-1 sm:ml-2 md:ml-3">
                        @if($archivo->es_visible)
                            <span class="px-1 sm:px-1.5 md:px-2 lg:px-2.5 py-0.5 sm:py-1 md:py-1.5 text-xs sm:text-sm md:text-base font-medium rounded-full text-green-800 bg-green-100 whitespace-nowrap">
                                Visible
                            </span>
                        @else
                            <span class="px-1 sm:px-1.5 md:px-2 lg:px-2.5 py-0.5 sm:py-1 md:py-1.5 text-xs sm:text-sm md:text-base font-medium rounded-full text-red-800 bg-red-100 whitespace-nowrap">
                                Oculto
                            </span>
                        @endif
                    </div>
                ...
                <div class="space-y-1 sm:space-y-1.5 md:space-y-2 lg:space-y-3">
                    <div class="text-xs sm:text-sm md:text-base lg:text-lg font-semibold text-gray-800 truncate" title="{{ $archivo->nombre }}">
                        {{ $archivo->nombre }}
                    </div>
                    ...
                    <div class="text-xs sm:text-sm md:text-base text-gray-600">
                        Formato: {{ strtoupper($archivo->tipo_archivo) }}
                    </div>
                </div>
                <div class="flex space-x-2 sm:space-x-3 md:space-x-4 pt-3 sm:pt-4 md:pt-5 mt-3 sm:mt-4 md:mt-5 border-t border-gray-100">
                    ...
                    <button type="button" 
                            onclick="showDeleteModal('Eliminar Catálogo de Archivo', '¿Estás seguro de que quieres eliminar el catálogo \"{{ $archivo->nombre }}\"? Esta acción no se puede deshacer.', 'form-delete-mobile-{{ $archivo->id }}')"
                ...
                <form id="form-delete-mobile-{{ $archivo->id }}" method="POST" action="{{ route('archivos.destroy', $archivo) }}" class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
            ...
            <div class="col-span-full bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 md:p-8 lg:p-10 text-center">
                <div class="text-gray-500">
                    ...
                    <p class="text-sm sm:text-base md:text-lg lg:text-xl">No hay catálogos</p>
                </div>
            </div>
            @endforelse
            </div>
        ...
        <x-modal-eliminar 
            id="modal-eliminar-archivo"
            title="Eliminar Catálogo de Archivo"
            message="¿Estás seguro de que quieres eliminar este catálogo de archivo? Una vez eliminado, todos sus datos se perderán permanentemente."
...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        console.warn('Error initializing archivos page JavaScript:', error);
    }
});
</script>
@endpush
